        ...
        #region Default Options Display
            ...
            foreach ($option in $Options) {
                if ($option -is [hashtable]) {
                    $label = $option.Keys -match '^l' | Microsoft.PowerShell.Utility\Select-Object -First 1
                    [string]$labelValue = $option[$label]
                    $help = $option.Keys -match '^h' | Microsoft.PowerShell.Utility\Select-Object -First 1
